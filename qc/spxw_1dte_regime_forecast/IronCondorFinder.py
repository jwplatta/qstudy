...
class IronCondorFinder:
    ...
    def find_iron_condor(self, contracts, spx_price):
        ...
        if call_spread is None:
            raise Exception("No valid call spread found: " + str(len(calls)))

        put_spread = self.find_initial_spread(puts, spx_price, straddle_price, "PUT")
        if put_spread is None:
            raise Exception("No valid put spread found: " + str(len(puts)))
    ...
    def find_initial_spread(self, contracts, spx_price, straddle_price, side):
        ...
        # Find the actual strike in contracts closest to target_strike
        short_leg = min(contracts, key=lambda x: abs(x.strike - target_strike))

        if not short_leg:
            raise Exception(f"No valid short leg found for {side} at target strike {target_strike}")
        ...
        if not long_leg:
            raise Exception(f"No valid long leg found for {side} at target strike {long_strike}")
