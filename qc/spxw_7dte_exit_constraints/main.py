...
class Spxw7dteExitConstraints(QCAlgorithm):
    """
    SPXW 7DTE iron condor strategy with profit-target and stop-loss exits.
    """
    ...
    STOP_LOSS_EXIT_LIMIT_OFFSET = 1.00
    ORDER_REFRESH_MINUTES = 1
    ENTRY_WALK_AFTER_UNCHANGED_REFRESHES = 2
    ENTRY_WALK_INCREMENT = 0.05
    ...
    def initialize(self):
        ...
        self.option.set_filter(
            lambda universe: universe.expiration(self.MIN_DTE, self.MAX_DTE).weeklys_only()
        )
        ...
        self.entry_order_manager = IronCondorEntryOrderManager(
            ...
            walk_after_unchanged_refreshes=self.ENTRY_WALK_AFTER_UNCHANGED_REFRESHES,
            walk_increment=self.ENTRY_WALK_INCREMENT,
        )
    ...
    def monitor_positions(self):
        ...
        if self.is_warming_up or self.exit_order_manager.is_pending:
            return

        if not self.is_regular_market_hours():
            return

        position_price = self.exit_order_manager.current_market_debit()
        if position_price is None:
            return

        current_pnl = round(self.trade["entry_credit"] - position_price, 2)
        ...
        if position_price >= self.trade["stop_loss_price"]:
            self.exit_order_manager.submit_exit_order(
                self.trade,
                position_price,
                f"Stop loss reached: exit debit ${position_price:.2f}",
                limit_offset=self.STOP_LOSS_EXIT_LIMIT_OFFSET,
            )

    def on_order_event(self, order_event):
        filled_trade = self.entry_order_manager.handle_order_event(order_event)
        if filled_trade:
            filled_trade["profit_target"] = round(
                filled_trade["entry_credit"] * self.PROFIT_TARGET, 2
            )
            filled_trade["stop_loss_price"] = round(
                filled_trade["entry_credit"] * self.STOP_LOSS_MULTIPLE, 2
            )
