"""v2: Skip period (skip=5) to reduce churn at signal tail.

v0 baseline: daily turnover ~17%, cost drag ~4.3%/yr.
Hypothesis: a 5-day skip between signal window and trade date means the signal
doesn't react to very recent 1-5 day moves, which tend to reverse and cause
position flips. This reduces noise-driven turnover without materially shrinking
the 20-day momentum effect.
...
study = (
    Study(name="short_horizon_momentum_skip5", universe=universe, benchmark=benchmark)
    .base_signal(short_horizon_sector_relative_signal(window=20, skip=5))
