"""v0: Baseline — short-horizon sector-relative momentum with sector/beta neutralization.

Signal: 20-day cumulative return minus sector average return (sector-relative momentum).
Filters: min price $5, min ADV $20M, top-150 liquid universe.
Positions: 20 long / 20 short, equal-weighted.
Risk: sector + beta neutralization (60-day window, 2 passes), rebalance every 10 days.
"""
...
# ---------------------------------------------------------------------------
# Data — download once at module level so run.py can import without re-fetching
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Position scaler: sector + beta neutralization
# ---------------------------------------------------------------------------
...
def build_short_horizon_momentum_w20() -> Study:
    return (
        Study(name="short_horizon_momentum_w20", universe=universe, benchmark=benchmark)
...
study = build_short_horizon_momentum_w20()
