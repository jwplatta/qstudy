"""v4: Tighter book (n=10/10) — fewer positions, less rebalancing churn.

v0 baseline: daily turnover ~17%, cost drag ~4.3%/yr.
Hypothesis: holding 20L/20S means many borderline names near the cutoff that
flip in and out frequently. Concentrating to 10L/10S means only the strongest
signals are held, reducing marginal-rank churn. The book is smaller so
individual position sizes are larger, but turnover per dollar should fall.
...
study = (
    Study(name="short_horizon_momentum_tight10", universe=universe, benchmark=benchmark)
