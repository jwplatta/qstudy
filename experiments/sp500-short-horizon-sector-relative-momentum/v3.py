"""v3: Signal smoothing — average signal over a 3-day window before ranking.

v0 baseline: daily turnover ~17%, cost drag ~4.3%/yr.
Hypothesis: the raw 20-day return signal is noisy day-to-day, causing frequent
rank flips at the margin of the long/short cutoff. Averaging the signal over the
last 3 days before ranking stabilizes the book, reducing turnover without
degrading the underlying information content.
...
def short_horizon_sector_relative_signal_smoothed(
    ...
    def signal_fn(**cache):
        ...
        # Average the signal over the last `smooth` days to dampen rank noise
...
study = (
    Study(
        name="short_horizon_momentum_smoothed3",
        universe=universe,
        benchmark=benchmark,
    )
    .base_signal(short_horizon_sector_relative_signal_smoothed(window=20, skip=0, smooth=3))
